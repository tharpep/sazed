...
- *Moved up from Tier 2: memory is only trustworthy if it can be corrected; one bad extraction that persists silently erodes trust in the whole system*

### 4. Session KB Ingestion Cleanup
...
- *Moved up from Tier 2: tagging session entries is a data modeling decision that gets harder to retrofit as the volume grows*

### 5. Conversation Search & Naming
...
## Future (Tier 2 — Important)

### 6. Memory Cleanup & Expiration
...
- *Moved up: high leverage for relatively low effort; sidesteps the entire chunk → embed → pgvector → rerank pipeline for large docs*

### 8. KB Write Operations (remaining)
...
- *Moved up from Tier 2: simple KB extension with high daily usefulness*
...
### 13. Readwise Reader Integration
...
### 16. Weekly Portfolio Update Automation
...
- *Moved down: product feature that depends on the agent core being stable; no urgency*

---

## Later (Tier 3 — Nice to Have)

### 17. Google Calendar Advanced
...
- Shares infrastructure with the Portfolio Update automation (#16)
- *Moved down: highest dependency count of anything on this list — needs stable memory, notifications, scheduler infra, and a solid agent core*

### 21. Location Context
- iOS Shortcuts integration to track current location
- Injected into system prompt for contextual awareness

### 22. Twitter/X API (Read-Only)
...
- *Moved to Tier 3: minor context injection, trivial to build, low impact relative to other items*
