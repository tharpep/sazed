...
async def load_relevant_memory(user_message: str) -> list[dict[str, Any]]:
    ...
    Capped at settings.memory_facts_per_type_limit most-recently-updated facts
    per fact_type — instruction facts in particular are self-generated by the
    reflection pass in session.py after any session with tool thrash, and
    personal/instruction are always-included categories, so without a cap this
    block grows unbounded and re-sent on every single turn forever.
    """
    ...
    rows = await pool.fetch(
        ...
        SELECT id, fact_type, key, value, confidence, source, created_at, updated_at
        FROM (
            SELECT *, ROW_NUMBER() OVER (PARTITION BY fact_type ORDER BY updated_at DESC) AS rn
            FROM agent_memory
        ...
        """,
        list(categories),
        settings.memory_facts_per_type_limit,
    )
